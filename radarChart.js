...
function RadarChart(id, data, options) {
	...
	// redefined data as it was before
	var tmpArr = [];
	tmpArr.push(data.data);
	data = tmpArr;

	//If the supplied maxValue is smaller than the actual one, replace by the max in the data
	var maxValue = Math.max(cfg.maxValue, d3.max(data, function(i){ return d3.max(i.map(function(o){return o.perc;}))}));
	// var maxValue = 1;
	...
	axisGrid.selectAll(".levels")
	   ...
	   .enter()
		...
		.style("fill", function(d,i) {
			if (i == 0) {
				switch(platform) {
					case "iTunes": return "url(#itunes)";
					case "Spotify": return "url(#spotify)";
					case "Others": return "url(#others)";
					default: return "url(#others)";
				}
			}
			return "none";
		})
	...
	var radarLine = d3.svg.line.radial()
		...
		.radius(function(d) { return rScale(d.perc); })
	...
	blobWrapper.selectAll(".radarCircle")
		...
		.attr("cx", function(d,i){ return rScale(d.perc) * Math.cos(angleSlice*i - Math.PI/2); })
		.attr("cy", function(d,i){ return rScale(d.perc) * Math.sin(angleSlice*i - Math.PI/2); })
